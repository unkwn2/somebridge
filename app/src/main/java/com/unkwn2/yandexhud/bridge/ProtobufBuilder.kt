...
object ProtobufBuilder {
    ...
    fun buildNew(
        ...
        // f28 — id манёвра. При камере (maneuver=0) шлём сырой 0 — чистит стрелку.
        val f28Val = when {
            maneuver == 0 -> 0L                            // камера/idle: сырой 0 = чистит стрелку
            f28Mapped -> gaodeToF28(maneuver).toLong()
            else -> maneuver.toLong()
        }
